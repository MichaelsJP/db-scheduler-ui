...
export const getTimeline = async (start: Date, end: Date): Promise<Timeline> => {
  const queryParams = new URLSearchParams();
  queryParams.append('start', start.toISOString());
  queryParams.append('end', end.toISOString());

  const response = await fetch(`${API_BASE_URL}/timeline?${queryParams}`, {
    method: 'GET',
    headers: {
  ...
  if (response.status == 401) {
    ...
    throw new Error(`Error fetching timeline. Status: ${response.statusText}`);
  }
